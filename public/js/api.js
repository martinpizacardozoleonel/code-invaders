const API = (() => {
  ...
  return {
    ...
    async saveSpeedrun(time){ const t=Math.round(Number(time)||0); const tok=localStorage.getItem(TOKEN_KEY); if(!tok) throw new Error('Inicia sesión para guardar récord'); try{ return await request('/api/speedrun',{method:'POST',body:JSON.stringify({time:t})}); }catch(e){ const m=(e&&e.message)||''; if(m.includes('Failed to fetch')||m.includes('fetch')||m.includes('NetworkError')||m.includes('Load failed')||m.includes('Network request failed')){ console.warn('[API] saveSpeedrun offline fallback',m); return Local.saveSpeedrun(tok,t); } throw e; } },

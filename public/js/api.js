const API = (() => {
  ...
  async function request(path,options={}){
    ...
    let data={}; let raw=''; try{ raw=await res.text(); try{ data=JSON.parse(raw); }catch(e){ data={}; } }catch(e){}
    if(!res.ok){
  ...
  return {
    ...
    async saveSpeedrun(time){ const t=Math.round(Number(time)||0); const tok=localStorage.getItem(TOKEN_KEY); if(!tok) throw new Error('Inicia sesión para guardar récord'); try{ return await request('/api/speedrun',{method:'POST',body:JSON.stringify({time:t})}); }catch(e){ const m=(e&&e.message)||''; if(m==='Failed to fetch'||m==='NetworkError'||m.includes('Network request failed')||m.includes('Load failed')||m.includes('ERR_CONNECTION')||m.includes('ERR_NETWORK')){ console.warn('[API] saveSpeedrun offline fallback',m); return Local.saveSpeedrun(tok,t); } throw e; } },
    ...
    async multiJoin(){ return request('/api/multi/join',{method:'POST'}); },
    async multiLeave(){ return request('/api/multi/leave',{method:'POST'}); },
    async multiReady(ready){ return request('/api/multi/ready',{method:'POST',body:JSON.stringify({ready})}); },
    async multiChatSend(text){ return request('/api/multi/chat',{method:'POST',body:JSON.stringify({text})}); },
    async multiAnswer(text){ return request('/api/multi/answer',{method:'POST',body:JSON.stringify({text})}); },

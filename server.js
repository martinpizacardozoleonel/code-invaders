...
const CHAT_BUBBLES=[
 {id:'none',name:'Sin burbuja',price:0,css:'',preview:'#1a2332'},
 {id:'neon_blue',name:'💙 Burbuja Neon Azul',price:0,css:'background:linear-gradient(135deg,#0d47a1,#00e5ff);border:1.5px solid #00e5ff;box-shadow:0 0 12px rgba(0,229,255,.6);',rarity:'rare'},
 {id:'fire',name:'🔥 Burbuja Fuego',price:0,css:'background:linear-gradient(135deg,#ff3d00,#ff6d00);border:1.5px solid #ff3d00;box-shadow:0 0 14px rgba(255,61,0,.6);animation:bubbleFire 1.2s ease-in-out infinite;',rarity:'epic'},
 {id:'galaxy',name:'🌌 Burbuja Galaxia',price:0,css:'background:radial-gradient(ellipse at 30% 20%,#7c4dff,#1a0b2e);border:1.5px solid #7c4dff;box-shadow:0 0 16px rgba(124,77,255,.5);animation:galaxySpin 3s linear infinite;',rarity:'epic'},
 {id:'emerald',name:'💚 Burbuja Esmeralda',price:0,css:'background:linear-gradient(135deg,#00c853,#69f0ae);border:1.5px solid #00e676;box-shadow:0 0 12px rgba(0,230,118,.5);',rarity:'rare'},
 {id:'gold',name:'💛 Burbuja Dorada',price:0,css:'background:linear-gradient(135deg,#ffd600,#ffab00);border:1.5px solid #ffd600;box-shadow:0 0 16px rgba(255,214,0,.5);',rarity:'rare'},
 {id:'void',name:'🕳️ Burbuja Vacío',price:0,css:'background:radial-gradient(ellipse at center,#1a0033,#000);border:1.5px solid #6a1b9a;box-shadow:0 0 18px rgba(106,27,154,.6);animation:voidPulse 2s ease-in-out infinite;',rarity:'epic'},
 {id:'rainbow',name:'🌈 Burbuja Arcoíris',price:0,css:'background:linear-gradient(90deg,#ff1744,#ffd600,#00e676,#00e5ff,#7c4dff);background-size:200% 100%;border:1.5px solid #fff;animation:rainbowShift 2s linear infinite;',rarity:'legendary'},
 {id:'diamond',name:'💎 Burbuja Diamante',price:0,css:'background:linear-gradient(135deg,#e1f5fe,#b3e5fc,#81d4fa);border:1.5px solid #40c4ff;box-shadow:0 0 18px rgba(64,196,255,.6);',rarity:'legendary'},
 {id:'glitch',name:'👾 Burbuja Glitch',price:0,css:'background:linear-gradient(135deg,#00e5ff,#ff00ff);border:1.5px solid #00e5ff;animation:glitchFrame .6s steps(2) infinite;',rarity:'legendary'},
 {id:'exclusive_00',name:'👑 Burbuja Maestra 00',price:0,css:'background:linear-gradient(135deg,#000,#1a1a2e,#ffd700);border:2px solid #ffd700;box-shadow:0 0 20px #ffd700,0 0 40px rgba(255,215,0,.4);animation:masterGlow 1.5s ease-in-out infinite;',rarity:'mythic'}
];
const LUCKY_ITEMS=[
...
app.post('/api/lucky/spin', async (req,res)=>{
 try{ const user=await findUserByToken(req); if(!user) return res.status(401).json({error:'No autenticado'}); ensureShopFields(user); const cost=1000; if((user.coins||0)<cost) return res.status(400).json({error:'Puntos insuficientes (necesitas '+cost+' pts)'}); user.coins-=cost; const item=pickLuckyItem(); if(item.type==='coins'){ user.coins=(user.coins||0)+item.amount; await pushNotification(user.id,'🎰 Lucky Royale',`Ganaste ${item.amount} pts`,'success'); } else if(item.type==='exp'){ user.exp=(user.exp||0)+item.amount; await pushNotification(user.id,'🎰 Lucky Royale',`Ganaste ${item.amount} EXP`,'success'); } else if(item.type==='bubble'){ if(!user.chatBubbles.includes(item.bubbleId)) user.chatBubbles.push(item.bubbleId); await pushNotification(user.id,'🎰 Lucky Royale',`Ganaste una burbuja de chat: ${item.bubbleId}`,'success'); } user.luckySpins=(user.luckySpins||0)+1; await updateUser(user); res.json({item,coins:user.coins,exp:user.exp,chatBubbles:user.chatBubbles,luckySpins:user.luckySpins}); }catch(e){ res.status(500).json({error:'spin error'}); }
});
